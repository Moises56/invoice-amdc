...
export class EstadoCuentaPage implements OnInit {
  ...
  onSearch(searchParams: SearchParams) {
    ...
    this.estadoCuentaService.consultarEstadoCuenta(consultaParams).subscribe({
      next: (response) => {
        ...
        this.presentToast(
          'Estado de cuenta consultado exitosamente.',
          'success'
        );
      },
      error: (err) => {
        ...
        if (err.status === 404) {
          this.searchError.set(
            'No se encontraron datos para los parámetros de búsqueda proporcionados.'
          );
          this.presentToast('No se encontraron datos.', 'warning');
        } else if (err.status === 400) {
          this.searchError.set(
            'Parámetros de búsqueda inválidos. Verifique los datos ingresados.'
          );
          ...
          this.searchError.set(
            'Error al consultar el estado de cuenta. Verifique su conexión.'
          );
          this.presentToast(
            'Error al consultar el estado de cuenta.',
            'danger'
          );
        }
      },
  ...
  private procesarRespuestaConsulta(response: ConsultaECResponseNueva) {
    ...
    if (
      response.tipoConsulta === 'dni' &&
      response.propiedades &&
      response.propiedades.length > 0
    ) {
      ...
      this.estadoCuenta.set(
        this.estadoCuentaService.convertirAFormatoLegacy(response, 0)
      );
    } else {
      ...
      this.estadoCuenta.set(
        this.estadoCuentaService.convertirAFormatoLegacy(response)
      );
    }
  }
  ...
  seleccionarPropiedad(propiedad: PropiedadDto, index: number) {
    ...
    if (response) {
      this.estadoCuenta.set(
        this.estadoCuentaService.convertirAFormatoLegacy(response, index)
      );
    }
  }
  ...
  async imprimirIndividual() {
    ...
    if (!isConnected) {
      this.presentToast(
        'No hay ninguna impresora conectada. Configure la impresora en Configuración > Bluetooth.',
        'danger'
      );
      return;
    }
    ...
    try {
      ...
      const receiptText =
        this.estadoCuentaPrinterService.formatEstadoCuentaIndividual(
          data,
          this.lastSearchParams
        );
      await this.bluetoothService.print(receiptText);
      this.presentToast(
        'Recibo individual enviado a la impresora exitosamente.',
        'success'
      );
    } catch (error) {
      console.error('Error al imprimir:', error);
      this.presentToast(
        'Error al imprimir el recibo. Verifique la conexión de la impresora.',
        'danger'
      );
    }
  }
  ...
  async imprimirGrupal() {
    ...
    try {
      ...
      const receiptText =
        this.estadoCuentaPrinterService.formatEstadoCuentaGrupal(
          response,
          this.lastSearchParams
        );
      ...
      await this.bluetoothService.print(receiptText);
      this.presentToast(
        'Recibo grupal enviado a la impresora exitosamente.',
        'success'
      );
    } catch (error) {
      console.error('Error al imprimir:', error);
      this.presentToast(
        'Error al imprimir el recibo grupal. Verifique la conexión de la impresora.',
        'danger'
      );
    }
  }
  ...
  async imprimirRecibo() {
    ...
    try {
      ...
      await this.bluetoothService.print(receiptText);
      this.presentToast(
        'Recibo enviado a la impresora exitosamente.',
        'success'
      );
    } catch (error) {
      console.error('Error al imprimir:', error);
  ...
  get tienePropiedades(): boolean {
    const response = this.consultaResponse();
    return (
      this.esDNI && !!response?.propiedades && response.propiedades.length > 0
    );
  }
